...
def get_score(cookies):
    ...
    t = time.time()
    total_json = requests.get("https://pc-proxy-api.xuexi.cn/delegate/score/get?_t=%d"%(int(round(t * 1000))), cookies=jar, headers={'Cache-Control': 'no-cache'}).content.decode("utf8")
